import agentsUninformed
import agentsInformed
import enviroment
import random
import sys
...
def simulation(results, size, obstacle_rate):
    ...
    agent1=agentsUninformed.Agent(env,"bfs")
    agent1.think()
    ...
    agent2=agentsUninformed.Agent(env,"ucs")
    agent2.think()
    ...
    agent3=agentsUninformed.Agent(env,"dfs")
    agent3.think()
    ...
    agent4=agentsUninformed.Agent(env,"dls")
    agent4.think()
    ...
    agent5=agentsInformed.Agent(env)
    agent5.think()
    exploreAmount5=len(agent5.exploredNodes)
    results.append(['astar',str(exploreAmount5)])
    env.resetEnviroment()
    #print(exploreAmount5,end="  ")
